...
def compute_reward2(rob, irs, action, left, middle, right, right_count, left_count):
    """Compute the reward based on the Robobo's movement and sensor readings.
    Args:
        rob: The Robobo instance (either SimulationRobobo or HardwareRobobo)
        irs: The IR sensor readings from the Robobo.
    ...
    for episode in range(num_episodes):
        ...
        for step in range(MAX_STEPS):
            ...
            if "forward" in action:
                ...
                forward_streak = 0
            irs = rob.read_irs()
            reward, left_count, right_count = compute_reward2(rob, [ir_reading for idx, ir_reading in enumerate(irs) if idx in [2, 3, 4, 5, 7]], action, left, middle, right, left_count, right_count)
